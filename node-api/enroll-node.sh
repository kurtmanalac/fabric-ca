...
then
    ...
    KEYFILE=$(find /app/data/fabric-ca-client/upmoadmin/tls/keystore -type f -name '*_sk')
    fabric-ca-client register --id.name $ENROLL_ID --id.secret $ENROLL_PW -u https://github-fabric-ca.railway.internal:7054 --id.type $ID_TYPE --tls.certfiles /app/data/fabric-ca-server/tls-cert.pem --tls.client.certfile /app/data/fabric-ca-client/upmoadmin/tls/signcerts/cert.pem --tls.client.keyfile $KEYFILE
    sleep 5
    ...
    fabric-ca-client enroll -u https://$ENROLL_ID:$ENROLL_PW@github-fabric-ca.railway.internal:7054 --mspdir $FABRIC_CA_CLIENT_HOME/$ENROLL_ID/msp --tls.certfiles /app/data/fabric-ca-server/tls-cert.pem $CUSTOM_CMD
    mkdir -p $FABRIC_CA_CLIENT_HOME/$ENROLL_ID/msp/tlscacerts
    cp $FABRIC_CA_SERVER_HOME/ca-cert.pem $FABRIC_CA_CLIENT_HOME/$ENROLL_ID/msp/tlscacerts/tls-cert.pem
