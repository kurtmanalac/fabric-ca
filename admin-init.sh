...
then
    ...
    fabric-ca-client enroll -u https://admin:adminpw@github-fabric-ca.railway.internal:7054 --mspdir $FABRIC_CA_HOME/ca-admin/tls --csr.hosts $RAILWAY_PRIVATE_DOMAIN,$RAILWAY_SERVICE_NAME --tls.certfiles /app/data/fabric-ca-server/tls-cert.pem --enrollment.profile tls
    find /app/data/fabric-ca/ca-admin/msp/keystore/ -type f -name '*_sk' -print0 | xargs -0 ls -t | tail -n +2 | xargs rm
    echo "Admin enrolled!"
